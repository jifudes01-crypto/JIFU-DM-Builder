  ...
  return (
    <main className="page-shell">
      <section className="luxury-panel mb-6">
        <p className="text-sm font-black uppercase tracking-normal text-gold-300">Step 2</p>
        <h1 className="mt-2 text-3xl font-black text-white">{team.name}｜選擇模板</h1>
        <p className="mt-3 text-base leading-7 text-slate-200">只顯示此團隊已上架的模板。選擇模板後即可進入編輯器。</p>
        <button type="button" className="btn btn-secondary mt-4" onClick={refreshEditorData}>
          重新整理模板
        </button>
      </section>

      {loading ? <div className="mb-6 rounded-xl border border-gold-300/40 bg-gold-50 p-4 font-bold text-navy-900">資料載入中...</div> : null}
      {message ? <div className="mb-6 rounded-xl border border-gold-300/40 bg-gold-50 p-4 font-bold text-navy-900">{message}</div> : null}
      ...
      {visibleTemplates.length ? (
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {visibleTemplates.map((template) => (
            ...
            >
              <div className="grid aspect-[4/3] place-items-center rounded-xl border border-line bg-slate-100 text-slate-500">
                {template.thumbnail_url || template.image_url ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={template.thumbnail_url || template.image_url} alt={template.name} className="h-full w-full rounded-xl object-cover" />
                ) : (
